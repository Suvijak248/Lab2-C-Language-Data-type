...
1.1  int บน ESP32 ใช้กี่ไบต์?
    ตอบ: มีขนาด 4 ไบต์ (32 บิต)
1.2 จากผลลัพธ์ นักศึกษาสังเกตเห็นอะไรเมื่อค่าเกินขอบเขตของ int บน ESP32? 
    ตอบ:  ค่าจะไม่ออกมาเป็นค่าที่ถูกต้อง แต่จะกลายเป็นเลขผิดเพี้ยนเพราะเกินขอบเขตที่เก็บได้
...
- นักศึกษาสังเกตเห็นความแตกต่างของความแม่นยำระหว่าง float และ double อย่างไร? สถานการณ์ใดที่คุณควรเลือกใช้ double แทน float?
  ตอบ: หากความแม่นยำเป็นสิ่งสำคัญมาก ควรเลือก double (หากแพลตฟอร์มรองรับ)
ถ้าทำงานบน ESP32 โดยเฉพาะ ใช้ float ก็พอ เพราะ double ก็ยังเป็น 4 ไบต์
...
- char ใช้กี่ไบต์? ค่าตัวเลข (ASCII value) มีความสัมพันธ์กับอักขระอย่างไร?
  ตอบ: ใช้ 1 ไบต์ (8 บิต) char คือ ตัวเลขที่แทนรหัส ASCII
...
- ตอบ: int, unsigned int ,long ,unsigned long ใช้4ไบต์ และlong long ,unsigned long long ใช้8ไบต์
- 2.บน ESP32, long มีขอบเขตเท่ากับ int หรือไม่? ชนิดข้อมูลใดที่คุณจะใช้หากต้องการเก็บค่าจำนวนเต็มบวกที่ใหญ่ที่สุด?
- ตอบ: เท่ากัน หากต้องการเก็บค่าจำนวนเต็มบวกที่ใหญ่ที่สุด ใช้ unsigned long long(8 ไบต์)
...
ตอบ: ใช้1 ไบต์ (8 บิต) ผลลัพธ์คือ 0 (เกิด overflow) เพราะ256 เกินขอบเขต
...
ตอบ: ถ้าต้องการให้ผลลัพธ์เป็นทศนิยม ต้อง แน่ใจว่าอย่างน้อยหนึ่งตัวในสมการเป็น float หรือ double
...
ตอบ: ถ้าต้องการแปลงชนิดข้อมูลเพื่อให้เหมาะกับการคำนวณ
